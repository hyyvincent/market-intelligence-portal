...
function App(){
 ...
 return <div className="shell">
  ...
  <main>
   ...
   <section className="panel stock-panel"><div className="panel-head"><div><b>02</b><h2>股市综合看板</h2><p>对应实际上市主体 · 最近交易日或盘中行情 · 不同币种不做市值合计</p></div><div className="market-actions"><span className="live"><i></i>{status}</span><button className={`market-refresh ${status==="正在刷新"?"loading":""}`} type="button" onClick={refreshMarketQuotes} disabled={status==="正在刷新"} title="手动刷新最新行情" aria-label="手动刷新最新行情"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 11a8 8 0 1 0-2.34 5.66M20 5v6h-6"/></svg><span>{status==="正在刷新"?"刷新中":"刷新"}</span></button></div></div>
    <div className="controls">{["全部","中国上市主体","海外母公司"].map(x=><button className={scope===x?"active":""} onClick={()=>setScope(x)} key={x}>{x}</button>)}</div>
    <div className="stock-list">{filtered.map(s=>{const q=quotes[s.symbol||s.fallbackKey]||{};const state=q.pct>0?"up":q.pct<0?"down":"neutral";return <a href={s.url} target="_blank" className="stock" key={s.name}><div><strong>{s.name}</strong><small>{s.ticker} · {s.market}</small></div><Spark values={q.trend} state={state}/><div className={`quote ${state}`}><strong>{Number.isFinite(q.price)?q.price.toFixed(2):"—"}</strong><span>{Number.isFinite(q.pct)?`${q.pct>0?"+":""}${q.pct.toFixed(2)}%`:s.currency}</span></div><div className="market-cap"><span>市值</span><strong>{formatMarketCap(s,q.cap)}</strong></div><em>{displayQuoteDate(q.date)||"来源页"} ↗</em></a>})}</div>
...
function Spark({values=[],state}){if(values.length<2)return <div className="spark empty">等待行情</div>;const min=Math.min(...values),max=Math.max(...values),span=max-min||1;const d=values.map((v,i)=>`${i?"L":"M"}${2+i*86/(values.length-1)} ${30-(v-min)*25/span}`).join(" ");return <svg className={`spark ${state}`} viewBox="0 0 90 34" preserveAspectRatio="none"><path d={d}/></svg>}
